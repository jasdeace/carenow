...
- **2026-05-20** Diagnostic deployed `list-gemini-models` (`--no-verify-jwt`) — confirms both `gemini-3.1-flash-lite` and `…-preview` are live. So the model swap wasn't actually causing failures.
- **2026-05-20** **Cross-lane fix (B's files)**: `c0d8cc6` — your 158aee7 refactor left three stray `modelId` references in response payloads (`analyze-meal:102`, `process-ocr-gemini:93,115`). At runtime that's `ReferenceError: modelId is not defined` → 400 on every call → why 검사결과 OCR and meal-OCR were failing (not the model name). All three sites now use the imported `GEMINI_MODEL`. Both functions redeployed and verified — the error responses now contain a clean Gemini 400 for bad input with `modelUsed: "gemini-3.1-flash-lite"` in the debug field, instead of crashing.
