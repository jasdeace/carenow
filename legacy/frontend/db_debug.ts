...
async function checkDB() {
  ...
  if (lovedOnes && lovedOnes.length > 0) {
    ...
    const { data: apiData1, error: apiErr1 } = await supabase
      ...
      .select('id, user_id, role, users:user_id(id, name_ko)')
      .eq('circle_id', testCircleId);
    console.log('API Query users(...):', JSON.stringify(apiData1, null, 2), apiErr1);
  }

  console.log('\n--- testing getGiverTakersList API query ---');
  const testGiverId = '4ff42095-4c7d-4b14-9e0c-26862853f319'; // Hj
  console.log('Using giverId:', testGiverId);
  
  const { data: memberData, error: memberError } = await supabase
    .from('care_circle_members')
    .select('circle_id')
    .eq('user_id', testGiverId)
    .eq('role', 'caregiver');
  ...
  if (memberData && memberData.length > 0) {
    ...
    console.log('API Query getGiverTakersList:', JSON.stringify(lovedOnesData, null, 2), lovedOnesError);
  }
}
